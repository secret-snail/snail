...
// will stack ID's with same XLABEL_ on top
#define TOC_CSV_BAR(ID_, XLABEL_) do { \
    clock_t toc##ID_ = clock(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,%s,overflow\n", __FUNCTION__, #ID_, XLABEL_); \
    } else { \
        MSG("SeNtInAl,bar,%s,%s,%s,%g\n", __FUNCTION__, #ID_, XLABEL_, (toc##ID_ - tic##ID_)/(double)CLOCKS_PER_SEC); \
    } \
} while (0)

#define TOC_CSV_GROUPED_BAR(ID_, XGROUP_) do { \
    clock_t toc##ID_ = clock(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,%s,overflow\n", __FUNCTION__, #ID_, XGROUP_); \
    } else { \
        MSG("SeNtInAl,bar,%s,%s,%s,%g\n", __FUNCTION__, #ID_, XGROUP_, (toc##ID_ - tic##ID_)/(double)CLOCKS_PER_SEC); \
    } \
} while (0)

#define TOC_CSV_BOX(ID_) do { \
    clock_t toc##ID_ = clock(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,no-x,overflow\n", __FUNCTION__, #ID_); \
    } else { \
        MSG("SeNtInAl,box,%s,%s,no-x,%g\n", __FUNCTION__, #ID_, (toc##ID_ - tic##ID_)/(double)CLOCKS_PER_SEC); \
    } \
} while (0)
...
#define WALL_TOC_CSV_XY_NORMALIZED(ID_, X_, DIV_) do { \
    time_point<high_resolution_clock> toc##ID_ = high_resolution_clock::now(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,%d,overflow\n", __FUNCTION__, #ID_, X_); \
    } else { \
        MSG("SeNtInAl,xy,%s,%s_normalized,%d,%g\n", __FUNCTION__, #ID_, X_, std::chrono::duration_cast<std::chrono::microseconds>(toc##ID_ - tic##ID_).count() / (1000000.0*DIV_)); \
    } \
} while (0)

// will stack ID's with same XLABEL on top
#define WALL_TOC_CSV_BAR(ID_, XLABEL_) do { \
    time_point<high_resolution_clock> toc##ID_ = high_resolution_clock::now(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,%s,overflow\n", __FUNCTION__, #ID_, XLABEL_); \
    } else { \
        MSG("SeNtInAl,bar,%s,%s,%s,%g\n", __FUNCTION__, #ID_, XLABEL_, std::chrono::duration_cast<std::chrono::microseconds>(toc##ID_ - tic##ID_).count() / 1000000.0); \
    } \
} while (0)

#define WALL_TOC_CSV_GROUPED_BAR(ID_, XGROUP_) do { \
    time_point<high_resolution_clock> toc##ID_ = high_resolution_clock::now(); \
    if (tic##ID_ > toc##ID_) { \
        ERROR_MSG("SeNtInAl,error,%s,%s,%s,overflow\n", __FUNCTION__, #ID_, XGROUP_); \
    } else { \
        MSG("SeNtInAl,grouped_bar,%s,%s,%s,%g\n", __FUNCTION__, #ID_, XGROUP_, std::chrono::duration_cast<std::chrono::microseconds>(toc##ID_ - tic##ID_).count() / 1000000.0); \
    } \
} while (0)

#define WALL_TOC_CSV_BOX(ID_) do { \
