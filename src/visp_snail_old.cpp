      ...
      d = new vpDisplayX(I2);
        ...
        while(!stop_localize) {
            ...
            if (vpDisplay::getClick(I2, false) || signal_stop) {
                stop_localize_mutex.lock();
                stop_localize = true;
                stop_localize_mutex.unlock();
                break;
            }
        }
...
int main(int argc, const char **argv)
{
  ...
  stop_localize = false;
  std::thread t(localize);
  for (;;) {
      localize_mutex.unlock();
      if (stop_localize) {
          break;
      }
      std::this_thread::sleep_for(std::chrono::milliseconds((int)(1000/localize_freq)));
  }
